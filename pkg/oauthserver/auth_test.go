...
func TestWithOAuth_loginProviders(t *testing.T) {
	...
	for _, tt := range []struct {
		...
		{
			name: "multiple login providers with escaped spaces",
			providers: []osinv1.IdentityProvider{
				newProvider("idp-no-spaces"),
				newProvider("idp%20escaped%20spaces"),
			},
		},
		{
			name: "multiple login providers with spaces in name causes panic",
			providers: []osinv1.IdentityProvider{
				newProvider("idp-no-spaces"),
				newProvider("idp with spaces"),
			},
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.expectPanic {
				assertPanics(t, func() {
					oauthServerConfig.WithOAuth(http.NewServeMux())
				})
